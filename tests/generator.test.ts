...
const sample = createGeneratorOutput({
  ...
  authRequired: true,
  roles: ["parent", "child"],
  successMetrics: ["parents approve completed chores in under one minute", "children can see current allowance"]
});
...
assert(sample.spec.integrations.some((integration) => integration.name === "email reminders"), "Expected planned email reminder integration.");
...
assert(sample.risks.some((risk) => risk.id === "risk-agent-collision"), "Expected agent collision guardrail risk.");

const promptRoles = sample.promptPack.prompts.map((prompt) => prompt.role);
includesAll(promptRoles.join("\n"), ["Foundation / architecture", "UI / UX", "Backend / integrations", "QA / tests / docs"], "Prompt roles");
...
includesAll(markdownSpec, ["# Family Chore Allowance App Spec", "## Core Features", "## Guardrails"], "Markdown spec");

const checklist = formatChecklistMarkdown(sample.reviewChecklist);
includesAll(checklist, ["# Family Chore Allowance Review Checklist", "## Agent Coordination", "## Required Approvals"], "Checklist");
...
assert(questions.some((question) => question.id === "critical-metrics"), "Expected dashboard-specific questions.");
