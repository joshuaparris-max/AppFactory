...
export function exportMarkdownSpec(spec: AppSpec, techPlan?: TechPlan): string {
  const lines = [
    ...
    "",
    "## Core Features",
    ...spec.coreFeatures.map((feature) => `- **${feature.name}** (${feature.priority}): ${feature.description}`),
    "",
    "## User Journeys",
    ...
    ...(spec.integrations.length
      ? spec.integrations.map((integration) => `- **${integration.name}:** ${integration.purpose} (${integration.mode})`)
  ...
  if (techPlan) {
    lines.push(
      ...
      "",
      "## Scaffold Plan",
      ...techPlan.scaffoldPlan.map((item) => `- \`${item.path}\` (${item.ownerRole}): ${item.purpose}`)
    );
  }
