...
export function generateAppSpec(answers: ClarifyingAnswers): AppSpec {
  ...
  const integrations = compactList([
    ...(answers.integrations ?? []),
    ...template.recommendedIntegrations,
  ]);
  ...
  const platforms: PlatformTarget[] = answers.platforms?.length
    ? answers.platforms
    : ['web', 'mobile-web'];
  ...
  const entities = template.dataEntities.map<EntitySpec>(entity => ({
    ...
    fields: [
      ...
      { name: 'updatedAt', type: 'date', required: true, description: 'Last update timestamp.' },
      {
        ...
        type: 'string',
        required: false,
        description: 'Workflow state for filtering and review.',
      },
      {
        name: 'metadata',
        type: 'json',
        required: false,
        description: 'Extensible safe metadata for early prototypes.',
      },
    ],
  }));
    ...
    entities.unshift({
      ...
      fields: [
        ...
        },
        {
          ...
          type: 'string',
          required: true,
          description: `One of ${roles.join(', ')}.`,
        },
        {
          name: 'isActive',
          type: 'boolean',
          required: true,
          description: 'Whether the user can access the app.',
        },
      ],
    });
  }
...
function createPermissions(roles: string[], authRequired: boolean): PermissionSpec[] {
    ...
    return [
      {
        ...
        cannot: [
          ...
          'Trigger live deployments or external writes.',
        ],
      },
    ];
  }
  ...
  return roles.map((role, index) => ({
    ...
    can:
      index === 0
        ? [
            'Create, edit, review, and export records.',
            'Invite or manage lower-privilege users when implemented.',
          ]
        : ['Use assigned workflows.', 'View records relevant to their role.'],
    cannot:
      index === 0
        ? [
            'Bypass audit notes for sensitive changes.',
            'Deploy to production without human approval.',
          ]
        : [
            'Edit admin settings.',
            "Access unrelated users' private records.",
            'Trigger live integrations without approval.',
          ],
  }));
}
...
function createAssumptions(
  ...
  return compactList([
    ...
    answers.deadline
      ? `Target timing: ${answers.deadline}.`
      : 'Timeline is flexible enough to prioritise a clean vertical slice.',
    answers.monetization
      ? `Monetization direction: ${answers.monetization}.`
      : 'Monetization is not required for the first build.',
    answers.existingAssets?.length
      ? `Existing assets: ${answers.existingAssets.join(', ')}.`
      : 'No existing assets are assumed.',
  ]);
}
...
function createOpenQuestions(answers: ClarifyingAnswers, type: AppType): string[] {
  ...
  if (type === 'game-prototype') {
    questions.push(
      'What single mechanic should prove the prototype is fun before more content is added?'
    );
  }
...
function createAcceptanceCriteria(features: FeatureSpec[], metrics: string[]): string[] {
  return compactList([
    ...
    ...features
      ...
      .map(feature => `${feature.name} is represented in the first build plan.`),
