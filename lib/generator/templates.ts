...
export const SEED_TEMPLATES: Record<string, SeedTemplate> = {
  dashboard: {
    ...
    coreFeatures: [
      ...
      },
      {
        name: 'Status alerts',
        description:
          'Configurable warnings for missing data, overdue work, and threshold breaches.',
        priority: 'should',
    ...
    questions: [
      templateQuestion(
        'critical-metrics',
        'Which 3-5 metrics must be visible first?',
        'Metric priority shapes the dashboard layout.',
        'long-text',
        true
      ),
      templateQuestion(
        ...
        'Freshness determines caching and integration complexity.',
        'single-choice',
        false,
        ['manual', 'daily', 'hourly', 'near real-time']
      ),
    ],
  },
  'learning-app': {
    ...
    risks: [
      ...
      'progress mechanics that reward clicking instead of learning',
    ],
    questions: [
      templateQuestion(
        'curriculum-scope',
        'What skill or topic should the first course teach?',
        'The first course scope controls content and assessment design.',
        'short-text',
        true
      ),
      templateQuestion(
        'feedback-style',
        'Should feedback be strict, coaching-oriented, or exploratory?',
        'Feedback tone changes the learning experience.',
        'single-choice',
        false,
        ['strict', 'coaching', 'exploratory']
      ),
    ],
  },
  'family-life-app': {
    ...
    description:
      'A practical household app for coordination, routines, reminders, and shared responsibilities.',
    ...
    coreFeatures: [
      ...
      },
      {
        name: 'Approval flow',
        description:
          'A lightweight review step for completed tasks, allowance, or important changes.',
        priority: 'must',
    ...
    questions: [
      templateQuestion(
        'household-rules',
        'What household rules or approvals should the app enforce?',
        'Rules define workflows and permissions.',
        'long-text',
        true
      ),
      templateQuestion(
        'reward-model',
        'Does the app need rewards, allowance, points, or simple completion?',
        'Rewards add state and family-specific expectations.',
        'single-choice',
        false,
        ['none', 'allowance', 'points', 'custom rewards']
      ),
    ],
  },
  'simple-business-app': {
    ...
    description:
      'A focused workflow app for a small business process such as bookings, clients, quotes, or jobs.',
    ...
    coreFeatures: [
      ...
      },
      {
        ...
        description:
          'A status-based workflow for enquiries, quotes, bookings, delivery, and follow-up.',
        priority: 'must',
    ...
    questions: [
      templateQuestion(
        'business-process',
        'Which business workflow should the first version own end-to-end?',
        'A narrow workflow creates a useful first release.',
        'long-text',
        true
      ),
      templateQuestion(
        'customer-facing',
        'Will customers use this app directly, or only staff?',
        'Customer-facing flows require different UX and security choices.',
        'boolean'
      ),
    ],
  },
  'game-prototype': {
    ...
    description:
      'A playable prototype with a focused loop, progression, readable feedback, and content hooks.',
    ...
    coreFeatures: [
      {
        ...
        description:
          'A short but complete loop with clear input, feedback, win/loss state, and replay.',
        priority: 'must',
      ...
      },
      {
        ...
        description:
          'A small unlock, upgrade, score, or narrative consequence that gives play meaning.',
        priority: 'must',
    ...
    questions: [
      templateQuestion(
        'core-fantasy',
        'What should the player feel powerful, clever, or tense doing?',
        'The player fantasy should drive mechanics before content volume.',
        'long-text',
        true
      ),
      templateQuestion(
        'session-shape',
        'How long should one complete prototype run last?',
        'Session length affects map, save, and progression design.',
        'single-choice',
        false,
        ['1-3 minutes', '5-10 minutes', '15-30 minutes']
      ),
    ],
  },
  'data-analyser': {
    ...
    questions: [
      templateQuestion(
        'data-format',
        'What data formats should the first version support?',
        'Format scope determines parser and validation choices.',
        'multi-choice',
        true,
        ['CSV', 'JSON', 'Excel', 'manual paste']
      ),
      templateQuestion(
        'analysis-output',
        'What decisions should the analysis help users make?',
        'Decision context prevents generic charts with no action.',
        'long-text',
        true
      ),
    ],
  ...
  'staff-training-app': {
    ...
    description:
      'A workplace training app with modules, checks, completion evidence, and manager visibility.',
    ...
    questions: [
      templateQuestion(
        'training-domain',
        'What job role or compliance topic should the first modules cover?',
        'Training topic affects content structure and evidence requirements.',
        'short-text',
        true
      ),
      templateQuestion(
        'evidence-needed',
        'What proof of completion needs to be retained?',
        'Evidence requirements affect data retention and audit logs.',
        'long-text'
      ),
    ],
  },
...
export const UI_REVIEW_SNIPPETS: Record<string, string> = {
  ...
  noSecretsReminder:
    'Reminder: never commit secrets. Use `.env.example` and store secrets in secure stores.',
  buildMustPass: 'Build & tests must pass before merge — CI should gate merges to main.',
  doNotOverwrite:
    "Do not overwrite another agent's work: read diffs, coordinate in PR comments, and ask for a handoff if needed.",
};
