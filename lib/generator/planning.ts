import {
  branchNameForAgent,
  FILE_OWNERSHIP_SUGGESTIONS,
  GLOBAL_AGENT_GUARDRAILS,
} from './guardrails';
import type {
  AgentRole,
  AgentTask,
  ...
  return [
    {
      ...
      mission:
        'Create the project skeleton, shared types, state boundaries, domain modules, and architecture notes that other agents can safely build on.',
    ...
    },
    {
      role: UI_ROLE,
      mission:
        'Build the main user experience around the primary journey with copyable outputs, clear states, and no dependency on live services.',
      ...
      suggestedFiles: [
        'src/app/page.tsx',
        'src/components/AppSpecView.tsx',
        'src/components/PromptPackView.tsx',
    ...
    },
    {
      role: BACKEND_ROLE,
      mission:
        'Implement safe service adapters, validation, persistence boundaries, and planned integrations without making live external calls by default.',
    ...
    },
    {
      role: QA_ROLE,
      mission:
        'Protect the build from coordination chaos with tests, review checklists, documentation, and release verification.',
...
export function estimateComplexity(spec: AppSpec): ComplexityEstimate {
  ...
  score += featureScore;
  rationale.push(
    `${spec.coreFeatures.length} core feature(s) and ${spec.secondaryFeatures.length} secondary feature(s) add ${featureScore} points.`
  );
  ...
  if (spec.type === 'game-prototype' || spec.type === 'data-analyser') {
    score += 12;
    rationale.push(
      `${spec.type} requires specialised interaction or analysis logic, adding 12 points.`
    );
  }
...
export function identifyRisks(spec: AppSpec): Risk[] {
  const risks: Risk[] = [
    {
      ...
      mitigation:
        'Use owned path suggestions, handoff notes, PR review, and explicit coordination before shared edits.',
    ...
    },
    {
      ...
      mitigation:
        'Keep GitHub, Vercel, AI, payment, email, and database calls behind disabled service interfaces.',
  ...
  if (spec.integrations.length > 0) {
    risks.push({
      ...
      mitigation:
        'Ship with mock adapters and document the request/response shape for later live work.',
      ownerRole: BACKEND_ROLE,
    });
  }
  ...
  if (spec.permissions.length > 1) {
    risks.push({
      ...
      mitigation:
        'Centralise permission rules in typed domain logic and test role-specific workflows.',
      ownerRole: FOUNDATION_ROLE,
    });
  }
  ...
  if (spec.type === 'data-analyser') {
    risks.push({
      ...
      mitigation:
        'Show data quality warnings, sample counts, missing values, and confidence limits.',
      ownerRole: QA_ROLE,
    });
  }
  ...
  if (spec.type === 'game-prototype') {
    risks.push({
      ...
      mitigation:
        'Test one complete loop with clear controls, feedback, win/loss state, and replay before expanding.',
      ownerRole: UI_ROLE,
    });
  }

  if (
    spec.primaryAudience.toLowerCase().includes('child') ||
    spec.idea.toLowerCase().includes('child')
  ) {
    risks.push({
      ...
      mitigation:
        'Minimise personal data, use mock records in demos, and document approval and retention rules.',
      ownerRole: BACKEND_ROLE,
    });
  }
...
function stackForSpec(spec: AppSpec): StackRecommendation {
  const dataLayer =
    spec.type === 'data-analyser'
      ? 'Local file parser plus planned Postgres/object storage boundary'
      : 'Postgres-compatible repository interface with in-memory mock';
  const aiLayer =
    spec.type === 'data-analyser' || spec.type === 'learning-app'
      ? 'AIService interface with mock summaries; live calls disabled'
      : 'AIService interface available but disabled';
  ...
  return {
    ...
    auth:
      spec.permissions.length > 1
        ? 'Role-aware auth boundary, mocked until provider is selected'
        : 'Optional auth boundary, disabled for local prototype',
...
function scaffoldForSpec(spec: AppSpec): ScaffoldItem[] {
  return [
    {
      path: 'lib/domain/types.ts',
      purpose: 'Shared app, entity, role, and workflow types.',
      ownerRole: FOUNDATION_ROLE,
    },
    {
      path: 'lib/domain/workflows.ts',
      purpose: 'Pure workflow rules for core features and permissions.',
      ownerRole: FOUNDATION_ROLE,
    },
    {
      path: 'lib/services/index.ts',
      purpose: 'Service interfaces for persistence, AI, GitHub, Vercel, and future integrations.',
      ownerRole: BACKEND_ROLE,
    },
    {
      path: 'lib/services/mockServices.ts',
      purpose: 'Safe mock adapters used by tests and local demos.',
      ownerRole: BACKEND_ROLE,
    },
    {
      path: 'src/components',
      purpose: 'Reusable UI components for primary screens and generated outputs.',
      ownerRole: UI_ROLE,
    },
    {
      path: 'src/app',
      purpose: 'App routes, layout, and first journey screens.',
      ownerRole: UI_ROLE,
    },
    { path: 'tests', purpose: 'Unit, smoke, and regression tests.', ownerRole: QA_ROLE },
    {
      path: 'docs',
      purpose: 'Architecture, handoff notes, integration plan, and review checklist.',
      ownerRole: QA_ROLE,
    },
